...
function renderCrumbs(){
  if(!bucket){$('crumbs').innerHTML='<span class="muted">Create a bucket to get started</span>';$('uploadBtn').disabled=true;$('uploadFolderBtn').disabled=true;$('createFolderBtn').disabled=true;$('bucketSettingsBtn').disabled=true;$('rebuildBucketBtn').disabled=true;$('bulkDeleteBtn').disabled=true;return;}
  $('uploadBtn').disabled=false;$('uploadFolderBtn').disabled=false;$('createFolderBtn').disabled=false;$('bucketSettingsBtn').disabled=false;$('rebuildBucketBtn').disabled=false;$('bulkDeleteBtn').disabled=false;const parts=prefix.split('/').filter(Boolean);let acc='';let html=`<button class="crumb-btn ${parts.length?'':'current'}" onclick="goPrefix('')">${esc(bucket)}</button>`;parts.forEach((part,i)=>{acc+=part+'/';html+=`<span class="crumb-sep">${icons['chevron-right']}</span><button class="crumb-btn ${i===parts.length-1?'current':''}" onclick="goPrefix(decodeURIComponent('${enc(acc)}'))">${esc(part)}</button>`;});$('crumbs').innerHTML=html;
}
...
async function loadObjects(more=false){
  ...
  try{const q=new URLSearchParams({bucket,prefix});if(more&&nextAfter)q.set('after',nextAfter);const data=await api('GET','/api/objects?'+q);objectItems.push(...data.common_prefixes.map(value=>{const name=value.slice(prefix.length).replace(/\/$/,'');return {type:'folder',key:value,name,empty_component:!name,size:null,last_modified_ms:null};}),...data.entries.filter(value=>value.key!==prefix).map(value=>({...value,type:'object',name:value.key.slice(prefix.length)})));nextAfter=data.is_truncated?data.next_after:null;$('moreWrap').classList.toggle('hidden',!nextAfter);renderObjects();}catch(e){toast('Could not load objects',e.message,false);renderObjects();}
}
function openFolderDialog(){if(!bucket)return;$('folderSubtitle').textContent=`Create in “${bucket}/${prefix}”.`;$('folderName').value='';setInlineError('folderError');$('folderDlg').showModal();setTimeout(()=>$('folderName').focus(),50);}
async function createFolder(event){
  event.preventDefault();const name=$('folderName').value.trim();
  if(!name){setInlineError('folderError','Enter a folder name.');return;}
  if(name.startsWith('/')){setInlineError('folderError','Folder names cannot start with /.');return;}
  if(name.replace(/\/+$/,'').includes('//')){setInlineError('folderError','Folder names cannot contain consecutive slashes.');return;}
  const button=$('folderAction');setBusy(button,true,'Creating…');
  try{const result=await api('POST','/api/folders',{bucket,prefix,name});$('folderDlg').close();toast('Folder created',result.key);await loadObjects();}
  catch(e){setInlineError('folderError',e.message);}
  finally{setBusy(button,false);}
}
function renderObjects(){
  renderCrumbs();const query=$('objectSearch').value.trim().toLowerCase();const visible=objectItems.filter(o=>(o.empty_component?'empty segment':o.name).toLowerCase().includes(query));visibleItems=visible;$('objectCount').textContent=bucket?`${visible.length}${nextAfter?'+':''} item${visible.length===1?'':'s'}`:'';
