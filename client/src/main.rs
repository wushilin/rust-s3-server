...
const DEFAULT_PART_SIZE: u64 = 256 * 1024 * 1024;
...
async fn transfer_object_between_s3(
...
async fn multipart_copy_s3_to_s3(
    source_client: &Client,
    source_bucket: &str,
    source_key: &str,
    target_client: &Client,
...
async fn download_object(source: &str, target: Option<PathBuf>) -> Result<()> {
    let parsed = parse_s3_url(source)?;
    let bucket = parsed
        .bucket
        .ok_or_else(|| anyhow!("bucket is required in source `{source}`"))?;
...
async fn list_s3_objects(client: &Client, bucket: &str, prefix: &str) -> Result<Vec<ListedObject>> {
    let mut token = None;
...
async fn client_for_alias(alias_name: &str) -> Result<(Client, Alias)> {
    let cfg = load_config().await?;
    let alias = cfg
        .aliases
        .get(alias_name)
...
fn env_alias(name: &str) -> Option<Alias> {
...
fn parse_s3_url(input: &str) -> Result<S3Url> {
...
fn join_s3_target(target: &str, rel: &str) -> String {
    format!(
        "{}/{}",
...
async fn load_config() -> Result<McConfig> {
...
async fn save_config(cfg: &McConfig) -> Result<()> {
    let path = config_path()?;
